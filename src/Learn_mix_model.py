...
def learn_mix_model(subset_idx, K=4, magic_thh = 0.49):
    subset_ls = np.where(subset_lb==subset_idx)[0]
    dict_file = Dict['Dictionary_sub'].format(200,subset_idx)
    
    with open(dict_file, 'rb') as fh:
        _, centers, _ = pickle.load(fh)
    
    for cls_lb in subset_ls:
        sim_fname = os.path.join(Feat['cache_dir'],'simmat','simmat_mthrh049_obj{}.pickle'.format(cls_lb))
        feat_fname = os.path.join(Feat['cache_dir'], 'pool4_feat_{}.pickle'.format(cls_lb))
        savename = os.path.join(root_dir,'mix_model','obj{}_K{}_notrain.pickle'.format(cls_lb, K))
        ...
        mat_dis = mat_dis1
        N = mat_dis.shape[0]
        print('total number of instances for obj {}: {}'.format(cls_lb, N))
